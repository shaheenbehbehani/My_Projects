#!/usr/bin/env python3
"""
Step 3c.3 – Finalization Patch (Include Cold/Light + Fix CF Candidates)
Creates properly stratified sample with cold/light users and fixes CF scoring issues.
"""
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(log_dir / "step3c3_finalization.log"),
...
class FinalizationEvaluator:
    """Finalization evaluator with proper cold/light stratification and fixed CF scoring."""
    ...
    def load_artifacts(self):
        ...
        try:
            ...
            # Load ratings data for proper stratification
            self.artifacts['ratings'] = pd.read_parquet(
                self.data_dir / "collaborative" / "ratings_long_format.parquet"
            )
    ...
    def validate_schemas(self):
        ...
        if not all(col in self.artifacts['ratings'].columns for col in required_ratings_cols):
            missing = [col for col in required_ratings_cols if col not in self.artifacts['ratings'].columns]
    ...
    def create_stratified_sample(self, n_users: int = 600, seed: int = 42):
        """Create properly stratified evaluation sample with cold/light users."""
        logger.info(f"Creating stratified sample with N={n_users}, seed={seed}")
        
        # Set random seed
        np.random.seed(seed)
        
        # Count ratings per user
        user_rating_counts = self.artifacts['ratings'].groupby('user_index').size().reset_index()
        user_rating_counts.columns = ['user_index', 'ratings_count']
        
        # Merge with user index map to get user IDs
        user_stats = user_rating_counts.merge(
        ...
        # Create ground truth for each user (last rating as holdout)
        ground_truth = []
        for user_idx in user_stats['user_index']:
            user_ratings = self.artifacts['ratings'][
                self.artifacts['ratings']['user_index'] == user_idx
            ].sort_values('rating', ascending=False)  # Sort by rating for better holdout
            
            if len(user_ratings) > 0:
                # Use last rating as holdout
                holdout_item = user_ratings.iloc[-1]['canonical_id']
                ground_truth.append({
                    'user_id': user_stats[user_stats['user_index'] == user_idx]['userId'].iloc[0],
    ...
    def run_finalization(self):
        ...
        logger.info("=" * 80)
        logger.info("Starting Step 3c.3 – Finalization Patch (Include Cold/Light + Fix CF Candidates)")
        logger.info("=" * 80)
        
        try:
            # A) Rebuild evaluation sample
            eval_users, ground_truth_df = self.create_stratified_sample(n_users=600, seed=42)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Step 3c.3 Finalization Patch')
    parser.add_argument('--alpha_grid', nargs='+', type=float, default=[0.0, 0.3, 0.5, 0.7, 1.0])
    
    args = parser.parse_args()
    
    try:
        evaluator = FinalizationEvaluator(alpha_grid=args.alpha_grid)
