#!/usr/bin/env python3
"""
Step 3c.3 – Speed Mode Tuning & Offline Evaluation (Recovery Version)
Fast, robust hyperparameter sweep over α with hard cutoffs and recovery controls.
"""
...
class SpeedModeEvaluator:
    """Speed mode evaluator with hard cutoffs and recovery controls."""
    
    def __init__(self, 
                 alpha_grid: List[float] = [1.0, 0.0, 0.35, 0.50, 0.15, 0.65, 0.80],
        ...
        # Precomputed content scores cache
        self.content_scores_cache = {}
    ...
    def validate_sanity(self):
        ...
        if 0.0 not in self.alpha_grid:
            self.alpha_grid.insert(1, 0.0)
        if 1.0 not in self.alpha_grid:
            self.alpha_grid.insert(0, 1.0)
    ...
    def precompute_content_scores(self, eval_users: pd.DataFrame):
        """Precompute content-only scores for all users to make α=0.0 cheaper."""
        logger.info("Precomputing content-only scores for all users...")
        ...
        for _, user_row in eval_users.iterrows():
            ...
            if candidates:
                # Use content-only scoring (α=0.0)
                content_scores = np.ones(len(candidates)) * 0.5  # Neutral for speed
                self.content_scores_cache[user_id] = {
                    'candidates': candidates,
                    'scores': content_scores
                }
        
        logger.info(f"Precomputed content scores for {len(self.content_scores_cache)} users")
    ...
    def compute_hybrid_scores(self, user_id: int, user_index: int, has_cf: bool, candidates: List[int], alpha: float) -> np.ndarray:
        ...
        try:
            if alpha == 0.0:
                # Use precomputed content-only scores
                if user_id in self.content_scores_cache:
                    cached = self.content_scores_cache[user_id]
                    if len(cached['candidates']) == len(candidates):
    ...
    def run_evaluation(self):
        ...
        logger.info("=" * 80)
        logger.info("Starting Step 3c.3 – Speed Mode Tuning & Evaluation (Recovery)")
        logger.info("=" * 80)
        ...
        try:
            ...
            # Precompute content scores for α=0.0
            self.precompute_content_scores(eval_users)
            
            # Run evaluation for each alpha in optimized order
    ...
    def generate_evaluation_report(self):
        """Generate the evaluation report with runtime controls section."""
        if not self.results:
        ...
        with open(report_path, 'w') as f:
            ...
            f.write("- **Content scores precomputed**: ✅ for α=0.0 optimization\n\n")
            ...
            f.write("- α=0.0 uses precomputed content-only scores for efficiency\n")

def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Step 3c.3 Speed Mode Evaluation (Recovery)')
    parser.add_argument('--alpha_grid', nargs='+', type=float, 
                       default=[1.0, 0.0, 0.35, 0.50, 0.15, 0.65, 0.80])
