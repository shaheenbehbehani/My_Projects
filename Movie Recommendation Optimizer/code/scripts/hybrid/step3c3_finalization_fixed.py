#!/usr/bin/env python3
"""
Step 3c.3 – Finalization (Cold/Light Included + Recall Lift, Real Artifacts Only)
Produces conclusive results with proper cold/light stratification and scoring fixes.
"""
...
# Configure logging
def setup_logging():
    """Setup logging for finalization evaluation."""
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(log_dir / "step3c3_finalization_fixed.log"),
...
class FinalizationFixedEvaluator:
    """Finalization evaluator with proper cold/light inclusion and scoring fixes."""
    ...
    def __init__(self, 
        ...
        self.user_activity_snapshot = None
    ...
    def load_artifacts(self):
        ...
        logger.info("Loading artifacts for finalization evaluation...")
        ...
        try:
            ...
            snapshot_path = self.derived_dir / "user_activity_snapshot.parquet"
            if not snapshot_path.exists():
                raise FileNotFoundError(f"User activity snapshot not found: {snapshot_path}")
            self.user_activity_snapshot = pd.read_parquet(snapshot_path)
    ...
    def validate_schemas(self):
        ...
        # Check user activity snapshot
        required_snapshot_cols = ['user_index', 'ratings_count']
        if not all(col in self.user_activity_snapshot.columns for col in required_snapshot_cols):
            missing = [col for col in required_snapshot_cols if col not in self.user_activity_snapshot.columns]
            raise ValueError(f"user_activity_snapshot missing required columns: {missing}")
    ...
    def build_fast_maps(self):
        ...
        # User index to user ID mapping
        self.maps['user_index_to_id'] = dict(
            zip(self.artifacts['user_index_map']['user_index'].astype(np.int32),
                self.artifacts['user_index_map']['userId'])
        )
    ...
    def create_hybrid_sample(self, n_users: int = 600, seed: int = 42):
        """Create hybrid sample with proper cold/light stratification."""
        logger.info("WARNING – Finalization restart: replacing CSR-only sample with hybrid sampler (CSR + raw counts cache)")
        ...
        try:
            ...
            # Merge user activity snapshot with user index map
            logger.info("Creating hybrid sample with cold/light users...")
            
            # Add user_id to activity snapshot
            user_activity = self.user_activity_snapshot.copy()
            user_activity['user_id'] = user_activity['user_index'].map(self.maps['user_index_to_id'])
            # Don't drop users without user_id - they might be cold/light users not in CF
            user_activity['user_id'] = user_activity['user_id'].fillna(user_activity['user_index'].astype(str))
            
            # Add CF coverage flag
            user_activity['has_cf'] = user_activity['user_id'].isin(self.maps['user_id_to_index'])
            ...
            user_activity['bucket'] = user_activity['ratings_count'].apply(assign_bucket)
            
            # Filter users with at least 2 ratings for holdout guarantee
            users_with_holdout = user_activity[user_activity['ratings_count'] >= 2].copy()
            ...
            for bucket, target_count in target_counts.items():
                bucket_users = users_with_holdout[users_with_holdout['bucket'] == bucket]
                if len(bucket_users) >= target_count:
                    sampled = bucket_users.sample(n=target_count, random_state=seed)
                else:
                    sampled = bucket_users
                    # Borrow from next higher activity bucket if needed
                    if len(sampled) < target_count:
                        remaining = target_count - len(sampled)
                        if bucket == 'cold' and 'light' in target_counts:
                            light_users = users_with_holdout[users_with_holdout['bucket'] == 'light']
                        ...
                        elif bucket == 'light' and 'medium' in target_counts:
                            medium_users = users_with_holdout[users_with_holdout['bucket'] == 'medium']
                            if len(medium_users) > 0:
                                additional = medium_users.sample(n=min(remaining, len(medium_users)), random_state=seed)
                                sampled = pd.concat([sampled, additional], ignore_index=True)
                        elif bucket == 'medium' and 'heavy' in target_counts:
                            heavy_users = users_with_holdout[users_with_holdout['bucket'] == 'heavy']
                            if len(heavy_users) > 0:
                                additional = heavy_users.sample(n=min(remaining, len(heavy_users)), random_state=seed)
                                sampled = pd.concat([sampled, additional], ignore_index=True)
            ...
            users_no_gt = len(user_activity) - len(users_with_holdout)
            
            logger.info(f"Sample composition cold={bucket_counts.get('cold', 0)/len(eval_users)*100:.1f}% "
    ...
    def _sampling_timeout_handler(self):
        ...
        logger.warning("Sampling watchdog timeout; downsizing to N=400")
        raise TimeoutError("Sampling step exceeded 3 min; downsizing to N=400")
    
    def create_ground_truth(self, eval_users: pd.DataFrame):
        """Create ground truth for sampled users."""
        logger.info("Creating ground truth for sampled users...")
        
        # Check if ground truth already exists and matches
        ground_truth_path = self.hybrid_dir / "ground_truth_speed.parquet"
        if ground_truth_path.exists():
            try:
                existing_gt = pd.read_parquet(ground_truth_path)
                existing_users = set(existing_gt['user_id'].unique())
                sampled_users = set(eval_users['user_id'].unique())
                if existing_users == sampled_users:
                    logger.info("Reusing existing ground truth")
                    return existing_gt
            except Exception as e:
                logger.warning(f"Failed to load existing ground truth: {e}")
        
        # Create ground truth from ratings
        ratings_path = self.data_dir / "collaborative" / "ratings_long_format.parquet"
        sampled_user_ids = set(eval_users['user_id'].unique())
        
        # Read ratings and filter for sampled users
        ground_truth_data = []
        
        # Read all ratings (it's not that large)
        logger.info("Loading ratings for ground truth creation...")
        ratings = pd.read_parquet(ratings_path)
        
        # Filter for sampled users
        sampled_user_indices = set(eval_users['user_index'].unique())
        ratings_filtered = ratings[ratings['user_index'].isin(sampled_user_indices)]
        
        if len(ratings_filtered) > 0:
            # Add user_id
            ratings_filtered['user_id'] = ratings_filtered['user_index'].map(self.maps['user_index_to_id'])
            ratings_filtered = ratings_filtered.dropna(subset=['user_id'])
            
            # For each user, use highest rated movie as holdout
            for user_id, user_ratings in ratings_filtered.groupby('user_id'):
                if len(user_ratings) > 0:
                    # Get highest rated movie
                    best_rating_idx = user_ratings['rating'].idxmax()
                    holdout_item = user_ratings.loc[best_rating_idx, 'canonical_id']
                    ground_truth_data.append({
                        'user_id': user_id,
                        'canonical_id': holdout_item,
                        'is_positive': True
                    })
    ...
    def compute_cf_scores(self, user_id: int, candidate_movies: List[str]) -> np.ndarray:
        ...
        # Per-user min-max normalization with epsilon for constant vectors
        if len(scores) > 0:
            if scores.max() > scores.min():
                scores = (scores - scores.min()) / (scores.max() - scores.min())
            else:
                scores = np.full_like(scores, 0.1)  # Small epsilon instead of zeros
    ...
    def apply_percentile_ranking(self, scores: np.ndarray) -> np.ndarray:
        """Apply percentile ranking to scores to avoid scale mismatch."""
        if len(scores) == 0:
            return scores
    ...
    def compute_hybrid_scores(self, alpha: float, cf_scores: np.ndarray, content_scores: np.ndarray) -> np.ndarray:
        """Compute hybrid scores using the blend formula with percentile ranking."""
        # Apply percentile ranking to both score types
        cf_ranks = self.apply_percentile_ranking(cf_scores)
        content_ranks = self.apply_percentile_ranking(content_scores)
        
        # Blend using percentile ranks
        hybrid_scores = alpha * cf_ranks + (1 - alpha) * content_ranks
    ...
    def compute_metrics(self, candidates: List[str], scores: np.ndarray, ground_truth: Set[str]) -> Dict[str, float]:
        ...
        # Sort candidates by score with deterministic tie-breaking
            ...
            for batch_start in range(0, len(eval_users), self.batch_size):
                ...
                try:
                    ...
                    for _, user_row in batch_users.iterrows():
                        ...
                        gt_count = user_row['ratings_count']
    ...
    def run_finalization(self):
        """Run the finalization evaluation."""
        logger.info("=" * 80)
        logger.info("Starting Step 3c.3 – Finalization (Cold/Light Included + Recall Lift)")
        logger.info("=" * 80)
        
        try:
            # A) Create hybrid sample
            eval_users = self.create_hybrid_sample(n_users=600, seed=42)
            self.artifacts['eval_users'] = eval_users
            
            # B) Create ground truth
            ground_truth_df = self.create_ground_truth(eval_users)
            ...
            # E) Evaluate acceptance gates
            self.evaluate_acceptance_gates()
            
            logger.info("Finalization evaluation completed successfully")
    ...
    def evaluate_acceptance_gates(self):
        ...
        for _, row in results_df.iterrows():
            logger.info(f"α={row['alpha']}: recall_at_10={row['recall_at_10']:.4f}, map_at_10={row['map_at_10']:.4f}, "
                      ...
                      f"light_recall={row['light_recall']:.4f}, oracle_recall_at_10={row['oracle_recall_at_10']:.4f}, "
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Step 3c.3 Finalization Fixed')
    parser.add_argument('--alpha_grid', nargs='+', type=float, default=[0.0, 0.3, 0.5, 0.7, 1.0])
    parser.add_argument('--run_id', type=str, default="finalization_fixed")
    
    args = parser.parse_args()
    
    try:
        evaluator = FinalizationFixedEvaluator(alpha_grid=args.alpha_grid, run_id=args.run_id)
        evaluator.run_finalization()
